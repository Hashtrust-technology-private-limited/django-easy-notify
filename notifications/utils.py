...
def get_notifications(user, notification_status = None):
    ...
    if notification_status not in list(dict(Notification.NotificationStatus.choices).keys()):
        ...
        return Notification.objects.prefetch_related('receivers').filter(receivers = user,notification_status = notification_status)
    

def mark_as_read(user):
    Notification.objects.filter(receivers = user,notification_status = Notification.NotificationStatus.unread ).update(notification_status = Notification.NotificationStatus.read)

def mark_as_unread(user):
    Notification.objects.filter(receivers = user,notification_status = Notification.NotificationStatus.read ).update(notification_status = Notification.NotificationStatus.unread)